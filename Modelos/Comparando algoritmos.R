...
# Calcular COVERAGE para un algoritmo
calcular_coverage <- function(model_name, algos, eval_scheme, matriz_rec, n = 10) {
  modelo <- Recommender(getData(eval_scheme, "train"),
                        method = algos[[model_name]]$name,
                        parameter = algos[[model_name]]$param)
  
  pred <- predict(modelo, getData(eval_scheme, "known"), type = "topNList", n = n)
  recommended_items <- unique(unlist(as(pred, "list")))
  
  total_items <- ncol(matriz_rec)
  return(length(recommended_items) / total_items)
}

# Calcular NOVELTY para un algoritmo
calcular_novelty <- function(model_name, algos, eval_scheme, matriz_rec, n = 10) {
  # Calcular popularidad de ítems en el conjunto completo
  item_popularity <- colCounts(matriz_rec)
  item_prob <- item_popularity / sum(item_popularity)
  ...
  pred <- predict(modelo, getData(eval_scheme, "known"), type = "topNList", n = n)
  recommended_lists <- as(pred, "list")
  
  recommended_freq <- table(unlist(recommended_lists))
  valid_items <- names(recommended_freq)[names(recommended_freq) %in% names(item_prob)]
  
  if (length(valid_items) == 0) return(NA)  # evitar error si hay vacío
  
  novelty_values <- -log2(item_prob[valid_items])
  avg_novelty <- mean(novelty_values, na.rm = TRUE)
  return(avg_novelty)
}

# ---------------- EJECUCIÓN -----------------

# Definir el número N de ítems recomendados (top-N)
top_n <- 10
...
for (nombre in names(algos)) {
  cat("Procesando:", nombre, "\n")
  
  cov <- calcular_coverage(nombre, algos, eval_scheme, matriz_rec, n = top_n)
  nov <- calcular_novelty(nombre, algos, eval_scheme, matriz_rec, n = top_n)
